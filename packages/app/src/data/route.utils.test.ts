...
describe('createTypedRoute', () => {
  it('`makePath` should accepts a list of variables with the defined types', () => {
    ...
    expect(route.makePath({ orderNumber: 1234, shipmentCode: 'ACDC' })).toEqual(
      '/orders/1234/ACDC'
    )
    expect(
      route.makePath({ orderNumber: 1234 }, 'firstname=John&lastname=Doe')
    ...
    expect(
      route.makePath(
        { orderNumber: 1234, shipmentCode: 'ACDC' },
        'firstname=John&lastname=Doe'
      )
    ).toEqual('/orders/1234/ACDC?firstname=John&lastname=Doe')
  })
})
